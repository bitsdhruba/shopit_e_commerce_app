  ...
  return (
    <div className="flex w-3/5 m-auto">
      <div className="w-[100%]">
        {cart.length > 0 ? (
					<div>	
						<div className="w-[100%] md:w-[100%] flex flex-col p-2 ">
							{cart.map((cartItem) => (
								<CartItem cartItem={cartItem} key={cartItem.id} />
							))}
						</div>
						
						<div className="w-1/2 w-[100%] md:w-[100%] mt-5  flex flex-col">
							<h1 className="font-semibold text-xl text-green-800 my-5">YOUR CART</h1>
							<h1 className="font-semibold text-5xl text-green-700  m-5">
								SUMMARY
							</h1>
							...
							<div>
								<button className="bg-green-700 hover:bg-purple-50 rounded-lg text-white transition duration-300 ease-linear m-5 border-2 border-green-600 font-bold hover:text-green-700 p-3 text-xl">
									Checkout Now
								</button>
							</div>
						</div>
